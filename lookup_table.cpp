...
void plt::build_sliding_table(uint64_t _arr[], int index, int index_inc, int inc_x, int inc_y) {
...
void plt::build_knight_king_table(uint64_t _arr[], int nt) {
...
void plt::build_pawn_table(uint64_t _arr[], int dir, bool captures) {
...
void plt::merge_table(uint64_t to_table[], uint64_t from_table1[], uint64_t from_table2[]) {
    for (int i = 0; i < 64; i++)
        to_table[i] |= from_table1[i] | from_table2[i];
}

void plt::set_zero(uint64_t table[]) {
    for (int i = 0; i < 64; i++)
        table[i] = 0;
}

uint64_t plt::solution(int idx, uint64_t Apieces) {
...
uint64_t plt::convert_to(uint64_t N, int a, int b) {
    uint64_t res = 0;
...
void plt::generate_sol_array(uint64_t _arr[8][256]) {
    for (int i = 0; i < 8; i++)
        for (int j = 0; j < 256; j++)
            _arr[i][j] = solution(i, j);
}

void plt::build_rook_bishop_table(uint64_t sol_arr[8][256], uint64_t _arr[8][258], uint64_t mod, int a, int b) {
