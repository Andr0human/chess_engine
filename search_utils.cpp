...
void moveOrderClass::initialise(MoveList& myMoves) {
    mCount = myMoves.size();
...
void moveOrderClass::insert(uint64_t idx, double time_taken) {
    moves[idx].second = time_taken;
}

void moveOrderClass::sortList(int best_move) {
    for (uint64_t i = 0; i < mCount; i++) {
...
void moveOrderClass::setMoveOrder(MoveList& myMoves) {
    for (uint64_t i = 0; i < mCount; i++) {
...
void moveOrderClass::mprint(chessBoard& cb) {
    for (uint64_t i = 0; i < mCount; i++) {
...
int moveOrderClass::get_move(uint64_t index) {
    return moves[index].first;
}

uint64_t moveOrderClass::moveCount() {
    return mCount;
}

void moveOrderClass::reset() {
    for (uint64_t i = 0; i < mCount; i++)
        moves[i].second = 0;
}
...
int search_data::min3(int _X, int _Y, int _Z) {
    if (_Y < _X) _X = _Y;
    if (_Z < _X) _X = _Z;
    return _X;
}

int search_data::max3(int _X, int _Y, int _Z) {
    if (_Y > _X) _X = _Y;
    if (_Z > _X) _X = _Z;
    return _X;
}

int search_data::best_move() {
    return move[depth - 1].first;
}

int search_data::last_depth() {
    return depth - 1;
}

double search_data::last_eval() {
    float eval = static_cast<float>(move[depth - 1].second) / 100.0f;
    return eval * static_cast<float>(2 * side2move - 1);
}

int search_data::eval() {
    return move[depth - 1].second;
}

void search_data::set_to_move(int pc) {
    side2move = pc;
}

void search_data::update(int _d, int _e, int line[]) {
    for (int i = 0; i < _d; i++) pvAlpha[i] = line[i];
    move[depth++] = std::make_pair(line[0], _e);
}

void search_data::reset() {
    max_ply = max_qs_ply = depth = 0;
    for (int i = 0; i < maxPly; i++) move[i] = std::make_pair(0, 0);
    for (int i = 0; i < maxPly; i++) pvAlpha[i] = 0;
}

void search_data::set_depth_zero_move(int __m) {
    depth = 0;
    move[depth++] = std::make_pair(__m, 0);
}

bool search_data::is_part_of_pv(int __m) {
...
bool search_data::use_verification_Search() {
...
bool search_data::move_verified() {
...
void search_data::init(int color, int zMove) {
    info.set_to_move(color);
    info.set_depth_zero_move(zMove);
}

void search_data::set_discard_result(int zMove) {
    if (zMove == -1)    
...
void thread_search_info::set(chessBoard &tmp_board, MoveList &tmp, int t_dep,
                             int ta, int tb, int tply, int pv_idx, int start)
{
...
uint64_t thread_search_info::get_index() {
...
std::pair<int, int> thread_search_info::result() {
...
void ponder_list::setList(MoveList &myMoves) {
    mCount = myMoves.size();
    for (size_t i = 0; i < myMoves.size(); i++)
        moves[i] = myMoves.pMoves[i];
}

void ponder_list::insert(int idx, int cmove, int ceval, int cline[]) {
...
void ponder_list::show(chessBoard &board) {
...
void ponder_list::index_swap(uint64_t i, uint64_t j) {
...
void test_position::generate_name(int index) {
...
std::vector<std::string> split(const string &__s, char sep) {
...
vector<string> strip(vector<string> list) {
    for (auto &word : list) word = strip(word);
    return list;
}

vector<uint64_t> to_nums(const vector<string>& list) {
...
vector<string> extract_argument_list(int argc, char *argv[]) {
...
void bits_on_board(uint64_t value) {
    std::string arr[8];
